<script lang="ts">
    ...
    import { Heart, FlagOff, EyeOff } from "lucide-svelte";

    const { id } = $props();

    const { user, title, urgency, price, payout_type, tags } = {
...
<div class="roach">
    ...
    <div class="actions">
        <IconButton icon={Heart} isToggle={true} activeColour="var(--pink)" />
        <IconButton icon={EyeOff} isToggle={true} />
        <IconButton icon={FlagOff} isToggle={true} />
    </div>

    <div class="info">
        <a href={`/roach/${id}`} class="title">{title}</a>
